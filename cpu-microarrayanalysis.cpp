...
int main(int argc, char *argv[]) {
  ...
  all_gene_data.reserve( data_row.renal_disease.size() + data_row.control.size() ); // preallocate memory
  all_gene_data.insert( all_gene_data.end(), data_row.renal_disease.begin(), data_row.renal_disease.end() );
  all_gene_data.insert( all_gene_data.end(), data_row.control.begin(), data_row.control.end() );
  std::vector<double> permutation_t_stats;
  for (i = 0; i < 1000; i++) {
    std::random_shuffle()
    permutation_t_stats.push_back()
  }
  ...
  std::cout << "Normal Standard Deviation: " << standard_deviation(normal)
            << std::endl;
  std::cout << "Students T-Statistic: " << students_t_stat(renal, normal)
            << std::endl;
